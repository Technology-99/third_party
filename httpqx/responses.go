...
func AllResponsesCtx(ctx context.Context, w http.ResponseWriter, v any, err error, isOnlyOk bool) {
	if err != nil {
		ErrorCtx(ctx, w.(http.ResponseWriter), err)
	} else {
		if !isOnlyOk {
			Ok(w.(http.ResponseWriter))
		} else {
			OkJsonCtx(ctx, w.(http.ResponseWriter), v)
		}
	}
}
